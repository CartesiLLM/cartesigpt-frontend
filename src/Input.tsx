...
export const Input: React.FC<IInputPropos> = (propos) => {
    ...
    const buyNFTFromPortal = async (token: string, buyer: string, amount: number) => {
        let erc721Address = "0x68E3Ee84Bcb7543268D361Bb92D3bBB17e90b838"
        let erc20Address = "0x2797a6a6D9D94633BA700b52Ad99337DdaFA3f52"
        try {
            if (rollups && provider) {
                const data = ethers.utils.toUtf8Bytes(`Buy NFT with (${buyer}) of ERC721 (${token}).`);
                //const data = `Deposited ${args.amount} tokens (${args.token}) for DAppERC20Portal(${portalAddress}) (signer: ${address})`;
                const signer = provider.getSigner();
                const signerAddress = await signer.getAddress()

                const erc20PortalAddress = rollups.erc721PortalContract.address;
                const erc721PortalAddress = rollups.erc721PortalContract.address;
                const erc20TokenContract = signer ? IERC20__factory.connect(erc20Address, signer) : IERC20__factory.connect(erc20Address, provider);
                const erc721TokenContract = signer ? IERC721__factory.connect(erc721Address, signer) : IERC721__factory.connect(erc721Address, provider);

                // query current allowance
                const currentAllowance = await erc20TokenContract.allowance(signerAddress, erc20PortalAddress);
                if (ethers.utils.parseEther(`${amount}`) > currentAllowance) {
                    // Allow portal to withdraw `amount` tokens from signer
                    const tx = await erc20TokenContract.approve(erc20PortalAddress, ethers.utils.parseEther(`${amount}`));
                    const receipt = await tx.wait(1);
                    const event = (await erc20TokenContract.queryFilter(erc20TokenContract.filters.Approval(), receipt.blockHash)).pop();
                    if (!event) {
                        throw Error(`could not approve ${amount} tokens for DAppERC20Portal(${erc20PortalAddress})  (signer: ${signerAddress}, tx: ${tx.hash})`);
                    }
                }
